...
def sample_image_inference(
    ...
    width = prompt_dict.get("width", 1056)
    height = prompt_dict.get("height", 1056)
...
def denoise(
    ...
    for t_curr, t_prev in zip(tqdm(timesteps[:-1]), timesteps[1:]):
        ...
        # yiren gai
        # packed_latent_height, packed_latent_width = ae_outputs.shape[2] // 2, ae_outputs.shape[3] * 3 // 2
        ...
        # replace latent yiren
        # img[:,:, :, :img.shape[3] // 3] = ae_outputs
...
def get_noisy_model_input_and_timesteps(
    ...
    num_split = NUM_SPLIT
    # noisy_model_input[:, :, :, :w//num_split] = latents[:, :, :, :w//num_split]
    noisy_model_input[:, :, 2*h//num_split : h, 2*w//num_split : w] = latents[:, :, 2*h//num_split:h, 2*w//num_split:w]
